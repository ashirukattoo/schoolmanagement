...
class AdminMain extends BaseController{
    ...
    ======================================================================================*/
        public function dashboard()  {    
            // Counts
            $students = $this->studentModel->countExcept(['Completed', 'Transfered']);
            ...
            $performance = $this->rawResultModel
                ->select('exams.exName as exam, AVG(rawresults.raScore) as avg_score')
                ->join('exams', 'exams.exID = rawresults.exam_id')
                ->groupBy('rawresults.exam_id')
                ->findAll();
        ...
        public function save_employee(){
            ...
            $data = [
                ...
                'empRole' => $this->request->getPost('role'),
            ...
            $name = $name.$this->request->getPost('lname');
            $this->employeeModel->insert($data);
            
            return redirect()->back()->with('success', 'Account of '.$name.' Created Successfully!');
    ...
    =======================================================================================*/
        ...
        public function export_students_list($class_id = null){
            ...
            $sheet->setCellValue($col++.'1','Index No.');
            ...
            foreach($students as $stu){
                ...
                $sheet->setCellValue($col++.$row, $stu['indexNo']);
        ...
        public function save_guardian() {
            ...
            if($use_existing == 1){
                ...
                // Create new guardian
                $data = [
                    'empname' => $request->getPost('guardian_name'),
                    'guSex' => $request->getPost('sex'),
                    'guOccupasion' => $request->getPost('occupation'),
                    'empEmail' => $request->getPost('email'),
                    'guRegisterd' => date('Y-m-d'),
                    'guStatus'=>'active'
                ];
    ...
    =================================================================================================*/
        protected function getClassMasterComment($total_score, $max_total){
            ...
            elseif ($percent >= 30) {
                ...
                return "Ufaulu duni sana. Anahitaji msaada wa haraka na ufuatiliaji wa karibu ili kuboresha matokeo.";
            }
        }
...
private function calculateGpaGrade($gpa){
    if($gpa >= 4.5) return 'A';
    if($gpa >= 3.5) return 'B';
    if($gpa >= 2.5) return 'C';
    if($gpa >= 1.5) return 'D';
    return 'F';
        ...
        //Analyse Examination
public function generateAnalytics($exam_id){
    $results = $this->compiledResultsModel
        ->select('compiledresults.*, students.stuID')
        ->join('students', 'students.stuID = compiledresults.student_id')
        ->where('compiledresults.exam_id', $exam_id)
        ->findAll();
    ...
    $grades = $this->rawResultModel->gradePerSubject($exam_id);

    $registered = count($results);
    $sat = $registered;
    $absent = 0;
    ...
    foreach($results as $r) {
        ...
        $division = $r['crDivision'];

        if(isset($gradeSummary[$division])) {
            $gradeSummary[$division]++;
        }
    }

    $classGpa = $totalGpa / $registered;
    $passPercent = ($passed / $registered) * 100;
    ...
    $data = [
        ...
        'passed' => $passed,
        'failed' => $failed,
        'gpa' => number_format($classGpa,2),
        'gpa_grade' => $this->calculateGpaGrade($classGpa),
        'pass_percent' => number_format($passPercent,2),
        'grade_summary' => json_encode($gradeSummary),
        'subject_summary' => null,
        'eastatus' => 'compiled'
    ];

    $existing = $this->analyticsModel->getAnalytics($exam_id);

    if($existing) {
        $this->analyticsModel->updateAnalytics($exam_id,$data);
    } else {
        $this->analyticsModel->createAnalytics($data);
    }

    return redirect()->to("admin/analytics/$exam_id")
                     ->with('success','Analytics generated successfully.');
}
public function viewAnalytics($exam_id){
    $data['analytics'] = $this->analyticsModel
                            ->getAnalytics($exam_id);

    return view('admin/exams/view_analytics', $data);
}       
        ...
        public function export_class_results_pdf($class_id, $exam_id){
            ...
            foreach ($results as $r) {
                ...
                $html .= '<tr><td class="student">' . esc($r['index']) . '</td>';
                foreach ($subjects as $sub) {
        ...
        public function report_cards_class($exam_id, $class_id, $Ins=' ', $op=null, $cl=null){
            ...
            foreach ($results as $r) {
                ...
                // --- HEADER ---
                $html .= '
                <div class="title">JAMUHURI YA MUUNGANO WA TANZANIA</div>
                <div class="title">HALIMASHAURI YA WILAYA BUKOBA</div>
                <div class="title">SHULE YA SEKONDARI LYAMAHORO</div>
                <div class="small"> TAARIFA YA MAENDELEO YA MWANAFUNZI –'.esc(strtoupper($exam['exName'])).' REPORT '.$year.'</div>
